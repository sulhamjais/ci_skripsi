...
class Pindahkampus extends CI_Controller {
	...
	public function index()
	{
		...
		$data=array('title'=>'Surat Pindah Kampus - Aplikasi Administrasi Persuratan Online',
					'isi'=>'admin/tampilan_pindahkampus',
	...
	public function suratselesai()
		{
			...
			$tanggal=date('Y-m-d');
			$this->model_permintaan->aprovesurat('pindahkampus',$id,$tanggal);
			//$email=$this->model_permintaan->getemail('pindahkampus',$id);
			//$this->send_mail($email[0]['email']);
			redirect(base_url("/pindahkampus"));
		}

		public function view_pindahkampus()
		{
			$nomorsurat = "";
			$namasurat='pindahkampus';
			$id = $this->uri->segment(3);
			...
			$ceknomorsurat=$this->model_permintaan->cetaksurat('pindahkampus',$id);    //ini mau diedit (liat aktifkuliah)
		...
		public function suratproses()
			{
				$id = $this->uri->segment(3);
				$tanggal=date('Y-m-d');
				$this->model_permintaan->prosessurat('pindahkampus',$id,$tanggal);
				redirect(base_url("/pindahkampus/view_pindahkampus/$id"));
			}
			public function updatenomorsurat(){
				$nosurat = $this->input->post('nosurat');
				$idnomor = $this->input->post('idnomor');
				$idsurat = $this->input->post('idsurat');
				$this->model_permintaan->updatenomorsurat('pindahkampus',$idnomor,$idsurat,$nosurat,$this->session->userdata('programstudi'));
		...
		public function suratkefakultas()
			{
				$id = $this->uri->segment(3);
				$tanggal=date('Y-m-d');
				$this->model_permintaan->kirimkefakultas('pindahkampus',$id,$tanggal);
				redirect(base_url("/pindahkampus"));
			}
			public function suratbatal()
				{
					$id = $this->uri->segment(3);
					$this->model_permintaan->batalsurat('pindahkampus',$id);
					redirect(base_url("/pindahkampus"));
				}
