...
class suratlain extends CI_Controller {
	...
	public function index()
	{
		...
		$data=array('title'=>'Surat Lain - Aplikasi Administrasi Persuratan Online',
					'isi'=>'admin/tampilan_suratlain',
	...
	public function suratselesai()
		{
			...
			$tanggal=date('Y-m-d');
			$this->model_permintaan->aprovesurat('suratlain',$id,$tanggal);
			//$email=$this->model_permintaan->getemail('suratlain',$id);
			//$this->send_mail($email[0]['email']);
			redirect(base_url("/suratlain"));
		}

		public function view_suratlain()
		{
			$nomorsurat = "";
			$namasurat='suratlain';
			$id = $this->uri->segment(3);
			...
			$ceknomorsurat=$this->model_permintaan->cetaksurat('suratlain',$id);    //ini mau diedit (liat aktifkuliah)
			...
			$data=array('title'=>'Surat Lainnya - Aplikasi Administrasi Persuratan Online',
						'isi'=>'surat/tampilan_suratlain',
						'nosurat'=> $nomorsurat,
		...
		public function suratproses()
			{
				$id = $this->uri->segment(3);
				$tanggal=date('Y-m-d');
				$this->model_permintaan->prosessurat('suratlain',$id,$tanggal);
				redirect(base_url("/suratlain/view_suratlain/$id"));
			}
			public function updatenomorsurat(){
				$nosurat = $this->input->post('nosurat');
				$idnomor = $this->input->post('idnomor');
				$idsurat = $this->input->post('idsurat');
				$this->model_permintaan->updatenomorsurat('suratlain',$idnomor,$idsurat,$nosurat,$this->session->userdata('programstudi'));
		...
		public function suratkefakultas()
			{
				$id = $this->uri->segment(3);
				$tanggal=date('Y-m-d');
				$this->model_permintaan->kirimkefakultas('suratlain',$id,$tanggal);
				redirect(base_url("/suratlain"));
			}
		public function suratbatal()
			{
				$id = $this->uri->segment(3);
				$this->model_permintaan->batalsurat('suratlain',$id);
				redirect(base_url("/suratlain"));
			}
